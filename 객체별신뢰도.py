...
###########################################################################로그 출력되는 부분####################################################################
                        if x_center is not None and y_center is not None:
                            if text_to_speech(str(object_details)):
                                # playsound(text_to_speech(str(object_details)))
...
def parse_opt():
    ...
    # parser.add_argument("--source", type=str, default="vid.mp4", help="file/dir/URL/glob/screen/0(webcam)")   ?????
    parser.add_argument("--source", type=str, default=ROOT / "video/working.mp4", help="file/dir/URL/glob/screen/0(webcam)")
    ...
    parser.add_argument("--conf-thres", type=float, default=0.25, help="신뢰 임계값")
